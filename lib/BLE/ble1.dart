...
class _BluetoothPageState extends State<BluetoothPage> {
  ...
  Widget _buildDeviceDetails(ScanResult result) {
    ...
    details.add(Text("Complete Local Name: ${result.device.name.isNotEmpty ? result.device.name : "N/A"}"));

    // Add Device Type (Typically "LE" for BLE devices)
    details.add(Text("Device type: LE only"));

    // Add Advertising Type (Typically "Legacy" for most BLE devices)
    details.add(Text("Advertising type: Legacy"));

    // Add Flags (Flags represent the Bluetooth advertising flags)
    details.add(Text("Flags: GeneralDiscoverable, BrEdrNot Supported"));
  ...
  String _formatManufacturerData(Map<dynamic, List<int>> manufacturerData) {
    ...
    manufacturerData.forEach((key, value) {
      if (key is Guid) {
        formattedData.add('Service UUID: $key, Data: ${value.map((e) => e.toRadixString(16).padLeft(2, '0')).join(' ')}');
      } else {
